...
export class GmailChannel implements Channel {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      if (err?.message === 'invalid_grant') {
        logger.warn(
          'Gmail OAuth token is invalid (invalid_grant). Gmail channel disabled. Re-authenticate to resume.',
        );
        this.gmail = null;
  ...
  private async pollForMessages(): Promise<void> {
    ...
    try {
      ...
      if (err?.message === 'invalid_grant') {
        logger.error(
          'Gmail OAuth token is invalid (invalid_grant). Stopping Gmail polling. Re-authenticate to resume.',
        );
        this.gmail = null;
        return;
      }
